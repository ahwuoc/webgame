...
<div class="section__content">
            ...
            <div class="top-nav-container">
                ...
                <button class="btn-weapon-tab" onclick="switchTab('weapon')">TOP NẠP</button>
                <button class="btn-tienbang-tab" onclick="switchTab('tienbang')">Top TrainFarm</button>
        ...
        fetch('/API/BXH/getRankingSucManh.php')
                ...
                .then(data => {
                    ...
                    html += '<thead><tr><th>HẠNG</th><th>NHÂN VẬT</th><th>TỔNG DAME</th></tr></thead>';
                    html += '<tbody>';
        ...
        fetch('/API/BXH/getRankingThoiVang.php')
            ...
            .then(data => {
                let html = '<table class="rank_table">';
                html += '<thead><tr><th>HẠNG</th><th>NHÂN VẬT</th><th>HIẾN MÁU</th></tr></thead>';
                html += '<tbody>';
                ...
                for (let i = 0; i < 10; i++) {
                    ...
                    html += `<td>${row.name}</td>`;
                    html += `<td>${formatvnd(row.tongnap)}</td>`;
                    html += '</tr>';
                }
        ...
        fetch('/API/BXH/getRankingNap.php')
            ...
            .then(data => {
                ...
                html += '</tbody></table>';
                document.getElementById('tienbang').innerHTML = html;
            })
            .catch(error => {
                console.error('Lỗi khi lấy dữ liệu từ API:', error);
                document.getElementById('tienbang').innerHTML = 'Không thể tải dữ liệu từ API.';
            });
...
//             });
        ...
        function formatNumber(number) {
            if (number > 1000000000) {
                return (number / 1000000000).toFixed(1) + ' tỷ dame';
            } else if (number > 1000000) {
                return (number / 1000000).toFixed(1) + ' triệu dame';
            } else if (number >= 1000) {
                return (number / 1000).toFixed(1) + ' nghìn dame';
            } else {
        ...
        function formatvnd(number) {
            if (number > 1000000000) {
                return (number / 1000000000).toFixed(1) + ' TỶ VNĐ';
            } else if (number > 1000000) {
                return (number / 1000000).toFixed(1) + ' TRIỆU VNĐ';
            } else if (number >= 1000) {
                return (number / 1000).toFixed(1) + ' NGHÌN VNĐ';
            } else {
                return number.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
            }
        }
