...
$url = "https://api.sieuthicode.net/historyapimbbank/eOpdJZuzBhaT-nGaNfj-Tnxv-eToS-THLt";
$ch = curl_init();
...
for ($x = 0; $x < $count; $x++) {
 ...
 //   $user_id = isset(explode(' ', $description)[1]) ? explode(' ', $description)[1] : "-"; 
    $user_id        = parse_order_id($description,'nrblue');
    ...
    echo "--";
     ...
     echo "</br>";
      ...
      //  continue;
        ...
        // }
        try {
         $stmt = $conn->prepare("UPDATE account SET danap = danap + :price * $giatri, danap = danap + :price* $giatri, danap = danap + :price, vnd = vnd + :price1  WHERE id = :username");
